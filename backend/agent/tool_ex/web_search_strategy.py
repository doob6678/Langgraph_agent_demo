...
class WebSearchStrategy:
    ...
    async def execute(self, state: Any, tool_args: Dict[str, Any], ctx: ToolExecutionContext) -> Any:
        ...
        if q:
            try:
                results = await loop.run_in_executor(None, lambda: ctx.search_service.search_web_sync(q, k, mode=search_mode))
            except Exception:
                results = []
        state.search_results = results
        if results:
            result = await ctx.web_search_invoke({"query": q, "max_results": k, "mode": search_mode, "_results": results})
        else:
            result = await ctx.web_search_invoke({"query": q, "max_results": k, "mode": search_mode})
        ...
        if fetch_all and results:
            ...
            if urls:
                pages = await loop.run_in_executor(None, lambda: ctx.search_service.batch_read_webpages_sync(urls, output_format="markdown", mode=search_mode))
                if pages:
                    merged = []
