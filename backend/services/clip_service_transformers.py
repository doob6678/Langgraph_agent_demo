...
class CLIPService:
    """基于Transformers的CLIP模型服务"""
    
    def __init__(self, model_name: str = "openai/clip-vit-base-patch32", device: str = None):
        """初始化CLIP服务
        
        Args:
            model_name: CLIP模型名称，默认为openai/clip-vit-base-patch32
    ...
    def encode_text(self, text: Union[str, List[str]]) -> np.ndarray:
        ...
        Returns:
            文本特征向量，形状为 (n_texts, 512) 对于clip-vit-base-patch32模型
        """
    ...
    def encode_image(self, image: Union[Image.Image, np.ndarray, bytes]) -> np.ndarray:
        ...
        Returns:
            图片特征向量，形状为 (1, 512) 对于clip-vit-base-patch32模型
        """
        # 处理不同类型的图片输入
