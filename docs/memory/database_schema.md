...
1.  **多部门与数据隔离**：引入 `dept_id` 和 `visibility` 字段，实现部门间的数据共享与员工间的隐私隔离。
...
| `dept_id` | BIGINT | 所属部门ID | FK -> tenants.id, Index |
...
| `dept_id` | BIGINT | 归属部门 | FK -> tenants.id |
...
> **核心策略**：对 `dept_id`, `user_id`, `visibility` 建立 **标量索引 (Scalar Index)**，利用 `Partition Key` (`dept_id`) 实现物理级别的查询加速与隔离。

### 3.1 长期记忆集合 (`long_term_memories`)
存储从对话中提取的事实、偏好和知识库。

*   **Collection Name**: `agent_long_term_memory`
*   **Partition Key**: `dept_id` (推荐，实现部门物理隔离)
...
| `dept_id` | Int64 | - | 部门ID | **Partition Key**, 标量索引 |
